...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('car_info', function (Blueprint $table) {
            $table->bigIncrements('Car_num'); // 차량 일련번호
            ...
            $table->string('Car_img_path',500)->nullable(); // 이미지 저장 경로 저장 컬럼
            ...
            $table->string('Car_maker',500); // 차량 제조사
            $table->string('Car_year',500);  // 차량 연식       
            $table->integer('Car_price');   //차량 가격            
            $table->string('Car_sell_location', 500); //차량 판매 중인 지역
            ...
            $table->string('Car_selling_method',1); // 판매 방식(0:일반, 1: 리스, 2:렌트, 3:카드론(*주요수익처*) )
            $table->date('Car_regist_date'); // 연식/등록일자 구분을 위한것
            $table->string('Car_model',500); // 차량 모델(g70,g80, m3,m4등등)
            $table->string('Car_grade',500); // 차량 등급(베이지, 노블, 프레, 인텔 등)       
            $table->string('Car_address',500); // 차량이 있는 위치 - 판매장 주소 또는 차량 위치
            $table->softDeletes();
        });
    }
